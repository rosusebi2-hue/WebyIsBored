...
test('defeats keep the entrance, preserve earned mastery once, and generate useful reports', () => {
  ...
  const report = JSON.parse(buildReport(g, store.settings(), ['example error'])); assert.equal(report.version, '2.2.0'); assert.equal(report.seed, 721); assert.equal(report.weapon, 'scrapsteel'); assert.deepEqual(report.route, ['first']); assert.deepEqual(report.errors, ['example error']);
});
