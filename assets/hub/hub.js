(() => {
  ...
  if (playable.length) {
    ...
    playable.forEach((game) => {
      ...
      if (localPath(game.cover)) artwork.src = game.cover;
      else artwork.remove();
      const tags = card.querySelector('.game-tags');
      tags.replaceChildren();
      (game.tags || []).forEach((tag) => { const item = document.createElement('li'); item.textContent = tag; tags.append(item); });
      const controls = card.querySelector('.controls-button');
      if (game.controls === 'stick-and-swing' && typeof dialog.showModal === 'function') {
        controls.hidden = false;
        controls.dataset.controls = game.controls;
    ...
    upcoming.forEach((game, index) => {
      ...
      card.querySelector('h3').textContent = game.title;
      upcomingFragment.append(card);
    });
    upcomingRoot.replaceChildren(upcomingFragment);
    if (!upcoming.length) {
      document.getElementById('coming-soon').hidden = true;
      document.querySelector('nav a[href="#coming-soon"]').hidden = true;
    }
  }
  let controlsOpener;
  ...
  dialog.addEventListener('close', () => {
    ...
    if (controlsOpener && controlsOpener.isConnected) controlsOpener.focus();
  });
